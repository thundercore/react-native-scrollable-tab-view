...
export interface ScrollableTabViewProperties extends React.Props<ScrollableTabView> {
    ...
    /**
     * tab可以滚动滑动，滚动到顶部后，tab可以固定在顶部
     * https://github.com/ptomasroos/react-native-scrollable-tab-view/pull/667
     * 但是有两个问题:
     * 1.因为ScrollView里面嵌套FlatList，导致FlatList的下拉刷新失效(onReachEnd不确定)
     * 2.如果多个tab的高度不一样(肯定会不一样)，从长的tab切换到短的tab，就显示一片空白
     */
